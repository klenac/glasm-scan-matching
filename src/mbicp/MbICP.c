...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Functions to draw correspondance points (Associations) by KL
// ---------------------------------------------------------------
// ---------------------------------------------------------------


// todo: drawing
// To draw correspondances for each iteration I save them
// in a file because I can use an external program to visualize them.


int write_associations(int numIteration)
{	const int FILENAME_SIZE = 19;
	...
	{	printf("write_associations: Error opening file");
		return 0;
	}

	for(i=0; i<cntAssociationsT; i++) {
		//printf("cntAssociationsT, filename %d,%s\n",cntAssociationsT,filename);
		if (fprintf(out,"%f,%f,%f,%f\n",cp_associations[i].nx,cp_associations[i].ny,cp_associations[i].rx,cp_associations[i].ry)<4)
		{	printf("write_associations: Error writing");
			break;
		}
	}
	if (fclose(out) !=  0)
	{	printf("write_associations: Error closing file");
	}
	return 1;
}
...
void Init_MbICP_ScanMatching(float max_laser_range,float Bw, float Br,
			...
			float error_x, float error_y, float error_t, int IterSmoothConv, int udraw_iterations){
...
for (i=0; i<MAXLASERPOINTS; i++) {
	...
	// printf("%d:, dist=%f, angle=%f,x=%f, y=%f\n",ptosNew.numPuntos,ptosNew.laserP[ptosNew.numPuntos].r,ptosNew.laserP[ptosNew.numPuntos].t,
	//     ptosNew.laserC[ptosNew.numPuntos].x,ptosNew.laserC[ptosNew.numPuntos].y);
		ptosNew.numPuntos++;
...
ptosNew.numPuntos=j;
// klenac 07.2010. todo: ptosNew.laserP is not updated during decimation??
...
for (i=0; i<MAXLASERPOINTS; i++) {
	...
	{
	//printf("laserK[i].r,MAXLASERRANGE %f, %f\n",laserK[i].r,MAXLASERRANGE);
	}
}
